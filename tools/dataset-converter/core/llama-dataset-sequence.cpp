...
static const int32_t* get_parquet_sequence_streaming(const struct llama_dataset* dataset, uint64_t index) {
    if (!dataset) {
        set_error("Dataset is null for Parquet streaming");
        return nullptr;
    }
    
    if (index >= dataset->n_seq) {
        set_error("Sequence index out of bounds for Parquet streaming");
        return nullptr;
    }
    ...
    set_error("Parquet streaming not fully implemented - requires format-specific data loading");
    return nullptr;
}

// Implementation of sequence function for all dataset types
const int32_t* sequence(const struct llama_dataset* dataset, uint64_t index) {
    ...
    if (index >= dataset->n_seq) {
        set_error("Sequence index out of bounds");
        return nullptr;
    }
    ...
    if (dataset->streaming) {
        switch (dataset->type) {
            case DATASET_GGUF: {
                ...
                void* streaming_data = gguf_get_tensor_data_streaming(dataset, index);
                if (!streaming_data) {
                    return nullptr;
                }
                ...
                if (dataset->cached_tensors && dataset->cached_tensors[index]) {
                    ...
                    free(streaming_data);
                    set_error("Tensor cache not properly initialized for streaming mode");
                    return nullptr;
                }
            ...
            case DATASET_PARQUET:
                ...
                return get_parquet_sequence_streaming(dataset, index);
            case DATASET_TEXT:
    ...
    if (!dataset->cached_tensors || !dataset->cached_tensors[index]) {
        set_error("Tensor cache not initialized or tensor not found");
        return nullptr;
    }
    ...
    if (dataset->streaming) {
        switch (dataset->type) {
            case DATASET_GGUF: {
                ...
                if (!dataset->ctx) {
                    set_error("GGUF context is null");
                    return 0;
                }
            ...
            case DATASET_PARQUET: {
                set_error("Parquet streaming length not implemented");
                return 0;
            }
            case DATASET_TEXT:
                // Text datasets don't support streaming yet, fall through to non-streaming path
...
struct ggml_tensor* sequence_tensor(const struct llama_dataset* dataset, uint64_t index) {
    ...
    if (dataset->streaming) {
        set_error("Direct tensor access not available in streaming mode");
        return nullptr;
    }
