...
void StreamingCache::evict_lru() {
    ...
    // Используем shared_lock для чтения статистики
...
void StreamingCache::evict_lfu() {
    ...
    std::shared_lock stats_lock(stats_mutex);
    auto             min_it =
        std::min_element(cache_list.begin(), cache_list.end(),
...
void StreamingCache::check_memory_pressure() {
    ...
    double memory_ratio = (double) current_memory_bytes.load() / max_memory_bytes;
    {
...
void StreamingCache::put(uint64_t sequence_id, void * data, size_t size) {
    ...
    if (existing_it != cache_map.end()) {
        ...
        LLAMA_LOG_DEBUG("Cached sequence %zu in streaming cache (size: %zu bytes, total: %zu/%zu bytes)", sequence_id,
...
size_t StreamingCache::get_max_memory() {
