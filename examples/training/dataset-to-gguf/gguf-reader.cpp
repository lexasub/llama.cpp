...
GGUFReader::GGUFReader(const std::string& path) : gguf_file_ptr(nullptr), file_path_(path) {
    try {
        ...
        std::cerr << "Error: Failed to initialize GGUFReader from file " << path << ": " << e.what() << std::endl;
...
std::string GGUFReader::get_metadata_str(const std::string& key, const std::string& defaultValue) const {
    if (!is_initialized()) {
        std::cerr << "Error: GGUFReader not initialized. Cannot get metadata string." << std::endl;
        return defaultValue;
    }
...
uint64_t GGUFReader::get_metadata_u64(const std::string& key, uint64_t defaultValue) const {
    if (!is_initialized()) {
        std::cerr << "Error: GGUFReader not initialized. Cannot get metadata u64." << std::endl;
        return defaultValue;
    }
...
int64_t GGUFReader::get_tensor_count() const {
    if (!is_initialized()) {
        std::cerr << "Error: GGUFReader not initialized. Cannot get tensor count." << std::endl;
        return 0;
    }
...
std::string GGUFReader::get_tensor_name(int64_t index) const {
    if (!is_initialized()) {
        std::cerr << "Error: GGUFReader not initialized. Cannot get tensor name." << std::endl;
        return "";
    }
    struct gguf_context* ctx_internal = gguf_file_ptr->get_gguf_context();
    if (!ctx_internal || index < 0 || index >= gguf_get_n_tensors(ctx_internal)) {
        return "";
    }
...
ggml_type GGUFReader::get_tensor_type(int64_t index) const {
    if (!is_initialized()) {
        std::cerr << "Error: GGUFReader not initialized. Cannot get tensor type." << std::endl;
...
size_t GGUFReader::get_tensor_size(int64_t index) const {
    if (!is_initialized()) {
        std::cerr << "Error: GGUFReader not initialized. Cannot get tensor size." << std::endl;
        return 0;
    }
    struct gguf_context* ctx_internal = gguf_file_ptr->get_gguf_context();
    if (!ctx_internal || index < 0 || index >= gguf_get_n_tensors(ctx_internal)) {
...
bool GGUFReader::read_tensor_data(int64_t index, std::vector<llama_token>& tokens) const {
    if (!is_initialized()) {
        std::cerr << "Error: GGUFReader not initialized. Cannot read tensor data." << std::endl;
        return false;
    }

    struct gguf_context* ctx_internal = gguf_file_ptr->get_gguf_context();
    if (!ctx_internal) {
        std::cerr << "Error: Internal GGUF context is null in GGUFFile." << std::endl;
        return false;
    }

    if (index < 0 || index >= gguf_get_n_tensors(ctx_internal)) {
        std::cerr << "Error: Tensor with index " << index << " not found." << std::endl;
        return false;
    }
    ...
    if (tensor_ggml_type != GGML_TYPE_I32) {
        std::cerr << "Error: Tensor type is not GGML_TYPE_I32 for tensor " << gguf_get_tensor_name(ctx_internal, index) << ". Actual type: " << ggml_type_name(tensor_ggml_type) << std::endl;
        return false;
    }
    ...
    if (!file.is_open()) {
        std::cerr << "Error: Could not open GGUF file '" << file_path_ << "' for reading tensor data." << std::endl;
        return false;
    }
    ...
    if (file.fail()) {
        std::cerr << "Error: Failed to seek to offset " << data_offset_in_file << " in file '" << file_path_ << "'. Stream state: good=" << file.good() << " eof=" << file.eof() << " fail=" << file.fail() << " bad=" << file.bad() << std::endl;
        file.close();
        return false;
    }
    ...
    if (!file) { // Check if the read operation failed or reached EOF before reading all bytes
        std::cerr << "Error: Failed to read " << expected_bytes << " bytes for tensor " << gguf_get_tensor_name(ctx_internal, index) << " from file " << file_path_ << std::endl;
        std::cerr << "  Stream state after read: good=" << file.good() << " eof=" << file.eof() << " fail=" << file.fail() << " bad=" << file.bad() << std::endl;
    ...
    if (file.gcount() != (std::streamsize)expected_bytes) {
        std::cerr << "Error: Mismatch in bytes read for tensor " << gguf_get_tensor_name(ctx_internal, index) << ". Expected " << expected_bytes << ", but read " << file.gcount() << "." << std::endl;
        file.close();
        return false;
    }
